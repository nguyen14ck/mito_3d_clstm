...
class TestAugmentor(object):
    ...
    def _tta_3d(self, model, data):
        ...
        for xflip, yflip, zflip, transpose in opts:
            ...
            # h5.savemat('volume.mat', {'volume':volume.cpu().numpy()})
            # h5.savemat('pred.mat', {'vout':vout.numpy()})
            # h5.savemat('volume.mat', {'volume':volume[0,0].cpu().numpy()})
            # h5.savemat('pred.mat', {'vout':vout[0].numpy()})
