...
class ConvGRUCell3D(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, bias):
        ...
        self.bias = bias
        self.update_gate = nn.Conv3d(in_channels=self.input_dim+self.hidden_dim, out_channels=self.hidden_dim,
                                     kernel_size=self.kernel_size, padding=self.padding,
                                     bias=self.bias)
        self.reset_gate = nn.Conv3d(in_channels=self.input_dim+self.hidden_dim, out_channels=self.hidden_dim,
                                    kernel_size=self.kernel_size, padding=self.padding,
                                    bias=self.bias)

        self.out_gate = nn.Conv3d(in_channels=self.input_dim+self.hidden_dim, out_channels=self.hidden_dim,
                                  kernel_size=self.kernel_size, padding=self.padding,
                                  bias=self.bias)
    ...
    def forward(self, input_tensor, cur_state):
        ...
        h_cur = cur_state
        # data size is [batch, channel, height, width, depth]
    ...
    def init_hidden(self, b, h, w, d):
        return torch.zeros(b, self.hidden_dim, h, w, d).cuda()
...
class ConvGRU3D(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, num_layers,
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(ConvGRUCell3D(in_channels=cur_input_dim,
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        if hidden_state is not None:
            ...
            b, _, _, h, w, d = input_tensor.shape
        ...
        for layer_idx in range(self.num_layers):
            ...
            h = hidden_state[layer_idx][0] # unpack list with 1 element
            output_inner = []
...
class ConvBGRU3D(nn.Module):
    # Constructor
    def __init__(self, in_channels, hidden_channels,
                 kernel_size, num_layers, bias=True, batch_first=False):

        super(ConvBGRU3D, self).__init__()
        self.forward_net = ConvGRU3D(in_channels, hidden_channels//2, kernel_size,
                                    num_layers, batch_first=batch_first, bias=bias)
        self.reverse_net = ConvGRU3D(in_channels, hidden_channels//2, kernel_size,
                                    num_layers, batch_first=batch_first, bias=bias)
    ...
    def forward(self, xforward, xreverse):
        ...
        y_out_fwd = y_out_fwd[-1] # outputs of last CGRU layer = B, T, C, H, W, D
        y_out_rev = y_out_rev[-1] # outputs of last CGRU layer = B, T, C, H, W, D
...
if __name__ == "__main__":
    ...
    x1 = torch.randn([2, 8, 60, 60, 60]).cuda()
    x2 = torch.randn([2, 8, 60, 60, 60]).cuda()
    x3 = torch.randn([2, 8, 60, 60, 60]).cuda()

    cbgru = ConvBGRU3D(in_channels=32, hidden_channels=64, kernel_size=(3, 3, 3), num_layers=1, batch_first=True).cuda()
