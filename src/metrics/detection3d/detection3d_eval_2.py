...
from .vol3d_eval_2 import VOL3Deval
...
def get_mAP_3d(gt_seg, pred_seg, predict_score=None, predict_heatmap=None, thres=None, output_name='map_output', do_txt=1, do_eval=1):
    """ 
    ...
    if predict_score is not None:
        # Nx2: pred_id, pred_sc
        pred_score = readh5(predict_score)
    elif predict_heatmap is not None:
        # pred_heatmap = readh5(predict_heatmap)
        pred_heatmap = np.array(predict_heatmap * 255).astype(np.uint8)
        r_id, r_score, _ = heatmap_to_score(pred_seg, pred_heatmap, channel=-1)
        pred_score = np.vstack([r_id, r_score]).T 
    else: # default: sort by size
        ui,uc = np.unique(pred_seg,return_counts=True)
        uc = uc[ui>0]
    ...
    if output_name=='':
        # output_name = args.predict_seg[:args.predict_seg.rfind('.')]
        output_name = '/output/eval/check'
    v3dEval = VOL3Deval(result_p, result_fn, pred_score_sorted, output_name=output_name)
    if do_txt > 0:
